...
func (hd *HD) WsStreamHCLogin2(ctx *fasthttp.RequestCtx) {
	...
	err := hd.WSU.Upgrade(ctx, func(ws *websocket.Conn) {
		...
		for {
			...
			if err = ws.WriteMessage(mt, []byte("Architect413")); err != nil {
				hd.WSCL.Lock()
				delete(hd.WSCL.CM, u)
				hd.WSCL.Unlock()
			...
			// 2022/09/24 22:39:04 write: write tcp4 127.0.0.1:8800->127.0.0.1:60578: write: broken pipe | 3 Times
			// 2022/09/24 22:39:05 Connection [a413-127.0.0.1:60578] has been closed
