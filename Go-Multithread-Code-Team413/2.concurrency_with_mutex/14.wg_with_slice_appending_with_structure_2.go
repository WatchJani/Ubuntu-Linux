...
func main() {
	...
	//================================================================
	for i:=0; i<4; i++ {
		go ExecUnit(&s1,wg,strconv.Itoa(i),strconv.Itoa(i+2),strconv.Itoa(i+4))			// (&s1,wg)
	}
...
func ExecUnit(s1 *struct { List []string; Mutex sync.Mutex }, waitGroup *sync.WaitGroup, l1... string) {
	defer waitGroup.Done()
	s1.Mutex.Lock()
	s1.List = append(s1.List, l1...)
	s1.Mutex.Unlock()
}
